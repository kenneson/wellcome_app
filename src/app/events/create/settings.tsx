...
import { Alert, Switch, Text, TextInput, TouchableOpacity, useWindowDimensions, View } from 'react-native';
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            if (apiError.code === 'HOST_PAYOUT_SETUP_REQUIRED') {
                ...
                Alert.alert('NÃ£o foi possÃ­vel publicar', apiError.message || 'Tente novamente em instantes.');
            }
        } finally {
            setSubmitting(false);
        }
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: '#FFF' }} edges={['top', 'bottom']}>
            ...
            <KeyboardAwareScrollView contentContainerStyle={{ paddingHorizontal: horizontalPadding, paddingBottom: 130 }} keyboardShouldPersistTaps="handled" style={{ flex: 1 }}>
                ...
                <Text className="text-sm text-gray-500 mt-1 mb-5">Confira como seu evento serÃ¡ apresentado antes de publicar.</Text>
                ...
                <View className="border border-gray-200 bg-white overflow-hidden mb-6" style={{ borderRadius: 8 }}>
                    ...
                    <View className="p-4">
                        <Text className="text-xl font-extrabold text-[#1A1A1A]">{creation.data.details.title || 'Evento sem tÃ­tulo'}</Text>
                        ...
                        <SummaryRow icon="restaurant-outline" text={`${creation.data.dishes.length} ${creation.data.dishes.length === 1 ? 'item no cardÃ¡pio' : 'itens no cardÃ¡pio'}`} />
                        <SummaryRow icon="people-outline" text={`${creation.data.details.maxGuests || 0} vagas Â· R$ ${creation.data.details.pricePerGuest || '0,00'} por pessoa`} />
                    </View>
                </View>
                ...
                <ReviewSection title="Cardapio" onEdit={() => router.push('/events/create/menu')}>
                    {creation.data.dishes.map((dish, index) => (
                        <View key={dish.id} className="py-2 border-b border-gray-100 last:border-b-0">
                            ...
                            <Text className="text-xs text-gray-500 mt-0.5">{dish.category.replace('_', ' ').toLocaleLowerCase('pt-BR')}{dish.description ? ` Â· ${dish.description}` : ''}</Text>
                        </View>
                    ))}
                ...
                <View className="gap-3 mb-6">
                    <SelectionCard
                        label="InscriÃ§Ã£o imediata"
                        description="ApÃ³s o pagamento, a participaÃ§Ã£o Ã© confirmada automaticamente."
                        selected={creation.data.details.accessType === 'OPEN'}
                        onPress={() => creation.setAccessType('OPEN')}
                    />
                    <SelectionCard
                        label="Requer minha aprovaÃ§Ã£o"
                        description="VocÃª analisa a solicitaÃ§Ã£o antes da confirmaÃ§Ã£o e do pagamento."
                ...
                <View className="flex-row justify-between items-end mb-3">
                    <View className="flex-1"><Text className="text-lg font-bold text-[#1A1A1A]">Perguntas aos convidados</Text><Text className="text-sm text-gray-500 mt-1">Opcional, atÃ© cinco perguntas.</Text></View>
                ...
                {creation.data.details.questions.map((question, index) => (
                    <View key={`${question.question}-${index}`} className="flex-row items-center border border-gray-200 px-4 py-3 mb-2" style={{ borderRadius: 8 }}>
                        <View className="flex-1"><Text className="text-sm font-bold text-[#1A1A1A]">{question.question}</Text><Text className="text-xs text-gray-500 mt-1">{question.required ? 'ObrigatÃ³ria' : 'Opcional'}</Text></View>
                ...
                <View className="bg-gray-50 border border-gray-200 p-4 mb-5" style={{ borderRadius: 8 }}>
                    <TextInput
                        ...
                        placeholder="Ex: VocÃª possui alguma alergia?"
                        value={newQuestion}
                        maxLength={180}
                        onChangeText={setNewQuestion}
                    />
                    <View className="flex-row items-center justify-between mt-3">
                        <View className="flex-row items-center"><Switch value={isRequired} onValueChange={setIsRequired} trackColor={{ false: '#D1D5DB', true: '#FF8C42' }} /><Text className="text-sm text-gray-700 ml-2">ObrigatÃ³ria</Text></View>
